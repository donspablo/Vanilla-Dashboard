...
! function (t, i) {
    ...
    function n(i) {
        ...
        if ("notifi-wrapper" !== e.getAttribute("id")) {
            ...
            if (o = /notifi-notification-([a-zA-Z0-9]+)/.exec(o)[1], n && m.notifications[o].options.dismissable) m.removeNotification(o);
            else {
...
if (ifUrlExists(dashURL)) {
    ...
    iframe.src = dashURL;
    iframe.id = "dashBoardWrapper";


    var iframe2 = document.createElement('iframe');
    iframe2.src = dashURL;
    iframe2.id = "dashBoardEditor";

    $("#tab-graphql *").remove();
    $("#tab-graphql").html(iframe + iframe2);
}
...
function resetStar() {
    _this = getEl();
    _this.start.show();
    _this.cloned = _this.animation.clone(true);
    _this.animation.remove();
    _this.animation = _this.cloned;
...
function StarWars() {
    ...
    _this.start.hide();
    _this.audio.play();
    _this.el.append(_this.animation);
...
var events = [
    ...
    /**
     * Dispatches a random dummy event.
     */
    var dispatchEvent = function () {
      var event = events[Math.floor(Math.random() * (events.length - 1)) + 1];
      dashboard.$app.trigger('dashboard.messageReceived', event);
    };
    
    window.dashboard = {
      $app: $('#app'),
      dispatcher: setInterval(dispatchEvent, 5 * 1000),
      endDemo: function () {
        return clearInterval(this.dispatcher);
      } };
    
    
    dashboard.$app.on('dashboard.messageReceived', function (e, msg) {
      console.log(msg);
    });
    
    $('#welcome').click(dispatchEvent);
    
    /**
     * Welcome component
     *
     * Given a region entrance or exit message, will output a welcome
     * (or farewell message) to a user.
     *
     * @param {String} data-region - The region we care about messages for
     * @param {String} data-message-welcome - The welcome message. {$user} parses to user name.
     * @param {String} data-message-farewell - The farewell message. {$user} parses to user name.
     */
    dashboard.$app.find('.welcome-component').each(function (i, currentWelcomeComponent) {
      var $this = $(currentWelcomeComponent),
      region = $this.data('region'),
      greetingMsg = $this.data('message-welcome'),
      farewellMsg = $this.data('message-farewell'),
      activeMessages = [],
      activeUsers = [];
    
      /**
       * Creates a new DOM node with the provided message.
       *
       * @param {String} message - The message to inject into the new DOM node.
       * @returns {Object} - The new jQuery node.
       */
      var createNewMsgNode = function createNewMsgNode(message) {
        return $('<h2></h2>').text(message);
      };
    
    
      /**
       * Determines whether or not a user is cached as active.
       * 
       * @param {String} user - The user ID to check.
       * @returns {Boolean} - Whether the user exists in the active user queue.
       */
      var isUserActive = function isUserActive(user) {
        return activeUsers.some(function (active) {
          return user === active;
        });
      };
      ...
      /**
       * Removes any existing messages (both in the DOM and activeMessages queue)
       * belonging to a specificied user.
       *
       * @param {String} user - The user ID to use to remove messages.
       */
      var removeExistingMessages = function removeExistingMessages(user) {
        $this.find('h2').each(function (i, currentH2) {
          var $h2 = $(currentH2);
          if ($h2.data('user') === user) {
            console.log('activeMessages before', activeMessages.length);
            activeMessages.splice(activeMessages.indexOf($h2), 1);
            $h2.remove();
            console.log('activeMessages after', activeMessages.length);
          }
        });
      };
      ...
      /**
       * Removes a message from the DOM as well as the activeMessages queue.
       */
      var removeMessage = function removeMessage() {
        var $node = activeMessages.shift();
        $node.remove();
      };
    
    
      /**
       * Parses and handles new events.
       * @param {Object} e - The event object dispatched by the event.
       * @param {Object} message - The message object we've received with the event.
       */
      var handleNewMessage = function handleNewMessage(e, msg) {
        if (msg.regionId !== region) {
          return true;
        }
    
        if (msg.type !== 'regionEntrance' && msg.type !== 'regionExit') {
          return true;
        }
    
        var $messageNode,userIndex,
        addMessage = false,
        user = msg.userId,
        event = msg.type,
        message = event === 'regionEntrance' ?
        greetingMsg.replace('{$user}', user) :
        farewellMsg.replace('{$user}', user);
    
        if (!isUserActive(user) && event === 'regionEntrance') {
          $messageNode = createNewMsgNode(message).data('user', user);
          activeUsers.push(user);
          addMessage = true;
        } else if (isUserActive(user) && event === 'regionExit') {
          $messageNode = createNewMsgNode(message);
          activeUsers.splice(activeUsers.indexOf(user), 1);
          removeExistingMessages(user);
          addMessage = true;
        }
    
        if (addMessage) {
          $this.append($messageNode);
          activeMessages.push($messageNode);
          setTimeout(removeMessage, 7.275 * 1000);
        }
      };
    
      // Set up event handlers
      dashboard.$app.on('dashboard.messageReceived', handleNewMessage);
    });
     ...
     *
     * Given a region entrance or exit message, keeps track of nearby users.
     *
     * @param {String} data-region - The region we care about messages for
     * @param {String} data-message - The displayed message. {$num} parses to user count.
     */
    dashboard.$app.find('.connected-users').each(function (i, currentConnectedUsers) {
      var $this = $(currentConnectedUsers),
      region = $this.data('region'),
      countMessage = $this.data('message'),
      activeUsers = [];
    
      /**
       * Update the UI based on the current number of active users.
       */
      var updateCount = function updateCount() {
        var message = activeUsers.length === 1 ?
        countMessage.replace('users', 'user') :
        countMessage;
    
        $this.find('h3').text(message.replace('{$num}', activeUsers.length));
      };
       ...
       * @param {Object} message - The message object we've received with the event.
       */
      var handleNewMessage = function handleNewMessage(e, message) {
        if (message.regionId !== region) {
          return true;
        }
    
        if (message.type === 'regionEntrance' && activeUsers.indexOf(message.userId) === -1) {
          activeUsers.push(message.userId);
          updateCount();
        }
    
        if (message.type === 'regionExit') {
          var userIndex = activeUsers.indexOf(message.userId);
          if (userIndex !== -1) {
     ...
     * Coffee component
     *
     * Tells when the last cup of coffee was brewed in the office.
     *
     * @param {String} data-message-brewed - The fresh brew message. {$time} parses to time event occurred.
     * @param {String} data-message-depleted - The depleted message. {$time} parses to time event occurred.
     */
    dashboard.$app.find('.coffee-brewed').each(function (i, currentCoffeeComponent) {
      var $this = $(currentCoffeeComponent),
      currentState = $this.data('state'),
      brewMessage = $this.data('message-brewed'),
      goneMessage = $this.data('message-depleted');
       ...
       */
      var handleNewMessage = function handleNewMessage(e, message) {
        if (message.type !== 'coffeeBrewed' && message.type !== 'coffeeDepleted') {
          return true;
        }
    
        if (message.type === 'coffeeDepleted' && currentState === message.type) {
          return true;
        }
    
        var newMessage = message.type === 'coffeeBrewed' ?
        brewMessage.replace('{$time}', message.when) :
        goneMessage.replace('{$time}', message.when);
    
        $this.data('state', message.type);
     ...
     *
     * Given a region entrance or exit message, tells whether or not a conference room is vacant or not.
     *
     * @param {String} data-empty-verbiage - Word to use for a room being available.
     * @param {String} data-full-verbiage - Word to use for a room being occupied.
     * @param {String} data-message - The message to use. {$room} gets parsed to the room name, {$status} gets parsed to the current status of the room.
     *
     * @param {String} h3#id - The child H3's must have an ID corresponding to those coming in via events.
     * @param {String} h3[data-room-name] - The room name to use for the associated room.
     */
    dashboard.$app.find('.conference-rooms').each(function (i, currentConferenceComponent) {
      var $this = $(currentConferenceComponent),
      stateMessage = $this.data('message'),
      emptyVerbiage = $this.data('empty-verbiage'),
      fullVerbiage = $this.data('full-verbiage'),
      state = {};
    
      // Fill up the state object
      $this.find('h3').each(function (i, currentConf) {
        state[currentConf.id] = {
          id: currentConf.id,
          $el: $(currentConf),
          name: $(currentConf).data('room-name'),
          users: [] };
    
      });
    
      /**
       * Check whether or not we care about the message coming in.
       */
      var isRelevantMessage = function isReleventMessage(message) {
        return (message.type === 'regionEntrance' || message.type === 'regionExit') &&
        state.hasOwnProperty(message.regionId);
      };
    
      /**
       * Update the UI based on the current state of the room provided.
       * @param {String} id - The ID of the room to update.
       */
      var updateMessage = function updateMessage(id) {
        var obj = state[id],
        roomMessage = stateMessage.replace('{$room}', obj.name),
        msg = obj.users.length > 0 ?
        roomMessage.replace('{$status}', fullVerbiage) :
        roomMessage.replace('{$status}', emptyVerbiage);
    
        obj.$el.html(msg);
      };
    
      /**
       * Parses and handles any new messages that come in.
       * @param {Object} e - The event object dispatched by the event.
       * @param {Object} message - The message object we've received with the event.
       */
      var handleNewMessage = function handleNewMessage(e, message) {
        ...
        // Cache the current room and cache whether the user exists
        var room = state[message.regionId],
        exists = room.users.some(function (user) {
          return message.userId === user;
        });
    
        if (message.type === 'regionEntrance') {
          if (!exists) {
            room.users.push(message.userId);
            updateMessage(room.id);
          }
        } else if (message.type === 'regionExit') {
          if (exists) {
            room.users.splice(room.users.indexOf(message.userId), 1);
            updateMessage(room.id);
          }
        }
      };
